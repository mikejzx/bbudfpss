...
#define BBUDFPSS_COMPR_LVL Z_BEST_COMPRESSION
...
{
public:
    z_stream_wrapper(bool _is_input = true, int _level = BBUDFPSS_COMPR_LVL)
        : is_input(_is_input)
    {
        this->zalloc = Z_NULL;
        this->zfree = Z_NULL;
...
public:
    ostreambuf(std::streambuf * _sbuf_p,
               std::size_t _buff_size = default_buff_size, int _level = BBUDFPSS_COMPR_LVL)
        : sbuf_p(_sbuf_p),
